...
export default function NextSessionPanel({
  ...
  const rangeEnd = useMemo(() => addDays(now, 45), [now]);
  ...
  return (
    <div className={`space-y-3 ${className}`}>
      <div className="flex items-center justify-between">
        <h3 className="text-base font-semibold text-slate-800">{panelTitle}</h3>
        {variant === 'list' && (
        ...
        <div className="space-y-3">
          {sessions.map((session) => {
            ...
            return (
              <div
                key={session.id}
                className={`relative overflow-hidden rounded-xl border bg-white p-4 shadow-sm transition-all duration-200 ${
                  highlight ? 'border-green-300 bg-emerald-50' : 'border-slate-200'
                }`}
              >
                <span
                ...
                <div className="pl-3">
                  ...
                  {session.groupId && (
                    <Link
                      ...
                      className="mt-3 inline-flex text-xs font-semibold text-green-700 hover:text-green-800"
                    >
                      Go to Group →
                    </Link>
                  )}
