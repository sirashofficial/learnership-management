# 📚 LESSON GENERATOR — DOCUMENT INTELLIGENCE PROMPT
# Education Platform — PDF/Word Document Ingestion + AI Lesson Generation
# ========================================================================
...
#
# YOUR BACKEND ALREADY HAS THESE ENDPOINTS — we just need to wire them up:
#   POST /api/ai/index-documents   ← Upload + process documents
#   GET/POST /api/ai/semantic-search ← Search inside documents
#   POST /api/groups/{id}/lessons/generate ← Generate using doc content
#
...
when generating lesson plans — not generic AI output.
...
1. UPLOAD PHASE: You upload your PDFs and Word docs into the system once.
   ...
   This is called "indexing" — it happens once per document.
...
2. SEARCH PHASE: When you ask for a lesson plan on "Unit Standard 3 - Health
   ...
   the relevant sections — learning outcomes, activities, assessment criteria.
...
Status values:
- "Indexed" (green) — processed and searchable
- "Processing" (yellow, animated) — currently being indexed
- "Failed" (red) — indexing failed, show retry button
- "Pending" (grey) — queued
...
- "Retry" button → calls POST /api/ai/index-documents with just that file's id
...
Look for:
- Does it call the AI with just the unit standard title? → WEAK (no document content)
- Does it call /api/ai/semantic-search first? → GOOD (using documents)
- Does it pass document chunks to the AI? → BEST
...
export async function POST(req: Request, { params }) {
  ...
  // This is the KEY step — find relevant sections from the uploaded PDFs
...
Do NOT invent content — base everything on the provided documents.
...
- 🟢 "X documents indexed — lesson will use your curriculum content"
- 🟡 "No documents indexed — lesson will use general AI knowledge only"
...
{generatedPlan.sourceDocuments?.length > 0 && (
  <div className="text-sm text-gray-500 mt-4">
    <p>📚 Generated using content from:</p>
    <ul>
...
## ═══════════════════════════════════════════════════════
## TASK 3 — DOCUMENT SEARCH PAGE (Curriculum Search)
## ═══════════════════════════════════════════════════════
...
```
┌─────────────────────────────────────────────────┐
│ 📄 Unit Standard 12433 — Health and Safety       │
│ Source: US_12433_Health_Safety.pdf               │
│ Relevance: ████████░░ 82%                        │
│                                                  │
│ "...the learner will be able to identify         │
│  hazards in the workplace and apply              │
│  appropriate safety procedures..."               │
│                                                  │
│ [Use in Lesson] [View Full Document]             │
└─────────────────────────────────────────────────┘
```
...
## ═══════════════════════════════════════════════════════
## TASK 4 — AI ASSESSMENT GENERATOR (Uses Documents)
## ═══════════════════════════════════════════════════════
...
"Save as Template" → POST /api/assessments/templates
"Create Assessment from This" → POST /api/assessments (creates actual assessment)

---

## ═══════════════════════════════════════════════════════
## TASK 5 — ADD DOCUMENT UPLOAD TO SETTINGS/ADMIN
## ═══════════════════════════════════════════════════════
...
- Last indexed date
- Quick link to "Manage Documents" → goes to Task 1 page
- "Re-index All" button (rebuilds all document indexes from scratch)
  → POST /api/ai/index-documents with { reindexAll: true }
...
### Option A — Bulk Script (Recommended for 30 files)
...
4. Get your auth token:
   ...
   - Press F12 (DevTools) → Application tab → Local Storage
   - Find the entry called 'token' and copy the value
5. Run the script:
   node scripts/bulk-upload-documents.js PASTE_YOUR_TOKEN_HERE
6. Watch the terminal — it will show each file uploading
7. When done, go to Admin → Knowledge Base to see all documents indexed

### Option B — Manual Upload Through UI

1. Go to Admin → Document Management (built in Task 1)
...
### Option C — Put Files in Public Folder (Development Only)
...
**Fix**: Check the backend indexing process — it may have crashed silently.
...
Flag these files to the user — they need to be re-created as text PDFs.
...
**Fix**: The indexing should chunk the document — check if there's a
