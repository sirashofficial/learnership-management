...
async function getOrCreateDefaultModule() {
  ...
  return prisma.module.create({
    data: {
      ...
      fullName: 'General Training',
      purpose: 'General training sessions',
...
async function seedTimetable() {
  try {
    ...
    const cityLogistics = findGroupByTokens(groups, ['city logistics', '26'], 'City Logistics (2026)');
    const azelisSa2026 = findGroupByTokens(groups, ['azelis', 'sa', '26'], 'Azelis SA (2026)');
    const monteagle2026 = findGroupByTokens(groups, ['monteagle', '26'], 'Monteagle (2026)');
    const beyondInsights = findGroupByTokens(groups, ['beyond insights', '26'], 'Beyond Insights (2026)');

    const flint2025 = findGroupByTokens(groups, ['flint', '25'], 'Flint Group (2025)');
    const packaging2025 = findGroupByTokens(groups, ['packaging', '25'], 'Packaging World (2025)');
    const azelis2025 = findGroupByTokens(groups, ['azelis', '25'], 'Azelis (2025)');
    const monteagle2025 = findGroupByTokens(groups, ['monteagle', '25'], 'Monteagle (2025)');
    const wahl2025 = findGroupByTokens(groups, ['wahl', '25'], 'Wahl (2025)');
