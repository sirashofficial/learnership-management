...
createGroupsContext() ← Custom context hook
     ...
     │       status, created_at, notes (plan),
     │       students: [],
     │       _count: { students, sessions }
     │      }
     │
...
For each group:
  ...
  2. CREDIT CALCULATION
     For each student:
       competentAssessments = assessments 
         WHERE status = 'COMPETENT'
       totalCredits = SUM(
         unitStandard.credits for each competent
       )
      
      Group Total = AVG(student credits)
