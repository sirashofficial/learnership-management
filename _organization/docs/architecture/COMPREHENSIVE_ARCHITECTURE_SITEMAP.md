...
GroupsContext fetches from the unified endpoint `/api/data/groups` and maps
server-computed metrics (progress, attendance, health) into the UI model.
...
#### **Unified Groups Data Flow (Current)**
- Single read source: `/api/data/groups`
- Server computes metrics in `src/lib/group-metrics.ts`
- Health status derived in `src/lib/statusUtils.ts`
- Rollout plan display built from `unitStandardRollouts`
  via `src/lib/rolloutUtils.ts`
...
│  - GroupsContext (group list + metrics)                    │
...
│  - useGroups() → SWR cache + unified /api/data/groups      │
