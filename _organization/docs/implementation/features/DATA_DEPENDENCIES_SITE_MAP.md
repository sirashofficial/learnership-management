...
│  ├─ Source: GET /api/data/groups (unified)
│  └─ Tables: Group, UnitStandardRollout, UnitStandard, Module,
│             Attendance (current month), Assessment (aggregated)
│
└─ Detail View (/groups/[id])
   ...
   Rollout Plan Resolution (UI):
   ├─ notes.rolloutPlan (JSON) → primary
   ├─ group.rolloutPlan (table) → fallback
   └─ unitStandardRollouts → buildRolloutPlanFromUnitRollouts()
...
SHARED CALCULATION + SYNC LOGIC
├─ Group metrics: src/lib/group-metrics.ts
│  ├─ avgCreditsPerStudent
│  ├─ avgProgressPercent
│  ├─ totalCreditsEarned / totalUniqueUnitsPassed
│  └─ currentAssessmentModule
├─ Health status: src/lib/statusUtils.ts
│  └─ calculatePerformanceStatus()
├─ Progress math: src/lib/progress-calculator.ts
├─ Rollout plan build: src/lib/rolloutUtils.ts
└─ Cache invalidation: src/lib/cache-invalidation.ts
   └─ invalidateGroups() + SWR mutate()
